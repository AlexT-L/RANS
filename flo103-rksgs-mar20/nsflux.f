...
c
      do i=2,il
         dx        = x(i,1,1)  -x(i-1,1,1)
         dy        = x(i,1,2)  -x(i-1,1,2)
         sigx      = .5*(q(i,1,1,1)  +q(i-1,1,1,1))
         sigy      = .5*(q(i,1,2,2)  +q(i-1,1,2,2))
         tauxy     = .5*(q(i,1,1,2)  +q(i-1,1,1,2))
         tw(i)     = (dx*dy*(sigx  -sigy)  -(dx**2  -dy**2)*tauxy)/
     .               (dx**2  +dy**2)
      end do
