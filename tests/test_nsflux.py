...
def test_eflux_validation():    
    ...
    model.test(ws,state,dw,'nsflux')

    # compare with fortran
    if UPDATE_FORTRAN_DATA:
        dw_fortran = Field.create(dw.shape)
        model.test(ws,state,dw_fortran,'nsflux','fortran')
        save('tests/validation/nsflux', dw_fortran)
    TOL = 1e-5
    dw_fortan = load('tests/validation/nsflux')
